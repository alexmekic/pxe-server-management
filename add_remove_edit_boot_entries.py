...
def add_restore_entry():
    ...
    while True:
        ...
        if new_image_entry == "x":
            ...
            break
        elif glob.glob("/pxe/images/" + image_list[int(new_image_entry)]):
...
def change_attached_restore_image():
    ...
    while True:
        ...
        if entry_id_edit == "x":
            ...
            if glob.glob('/pxe/images/' + (image_list[int(new_attached_image)])):
                with open('/pxe/tftp/boot.ipxe', 'r') as f:
                    for line in f:
                        if line.startswith('kernel'):
                            image_scan = re.search(r'\b'+image_list[int(new_attached_image)]+'\s', line)
                            if image_scan:
                                matched_images.append(image_scan.group(0))
                if not matched_images:
                    ...
                    print(image_list[int(new_attached_image)] + " already attached to another entry")
            else:
                print("Invalid image index or image file not found")
        else:
